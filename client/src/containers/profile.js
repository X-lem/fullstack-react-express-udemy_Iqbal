...
class Profile extends Component {
  ...
  componentDidMount() {
    // console.log("Profile - componentDidMount: " + JSON.stringify(this.props.db_profile))
  ...
  RenderPosts = post => (
    <Card style={{width: '500px', height: '200px', marginButton: '10px', paddingBottom: '80px' }}>
      <CardHeader 
        title={ <Link to={{pathname: '/post/' + post.post.id, state: {post}}}>
                  {post.post.title}
                </Link> 
              }
        subheader={
...
function mapStateToProps(state) {
  return {
    ...
    user_posts: state.post_reducer.user_posts,
